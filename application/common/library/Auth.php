...
class Auth
{
   ...
   public function wechatminilogin($openid,$session_key)
    {   
        $field = 'openid';
        $user = User::get([$field => $openid]);
        ...
        else
        {
            ...
            //记录本次登录的IP和时间,token、session_key
            $user->loginip = $ip;
            $user->logintime = $time;
            $user->token = md5($this->_token . $session_key);
     ...
     * @param string $rawData     用户信息
     * @param string $mobile     手机
     * @param string $password    
     * @param string $pid     上级userid
     * @param array $extend    扩展参数
     * @return boolean
     */
    public function wechatminiauth($openid, $session_key, $rawData = [], $mobile,$password,$pid,$extend = [])
    {
        $check = User::get(['openid'=>$openid]);
        if($check)
        {   //已经授权过
            return 666;
        }
        $checkmobile = User::get(['mobile'=>$mobile]);
        if($checkmobile)
        {   //此号码已经注册
            return 777;
        }
        $ip = request()->ip();
        $time = time();
        $username = emoji_encode($rawData['nickName']);
        ...
        try
        {
            ...
            $this->_user->token = md5($this->_token . $session_key);
            $this->_user->save();
